...
`AstroFITS` relies on code in `CFITSIO.jl` to call the C functions of the CFITSIO library.
...
and shipped with the `AstroFITS` package. The compatibility settings for the artifact
...
When it is loaded (by `using` or `import`), the `AstroFITS` package checks whether the API
of the CFITSIO library may have changed assuming the rules of [Semantic
...
``` julia
cd AstroFITS.jl/gen # go to the directory where is the code generator
...
2. update the `[compat]` entry in `AstroFITS.jl` project file [`../Project.toml`](../Project.toml),
